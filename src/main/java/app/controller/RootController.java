...
public class RootController {
    ...
    public String root(Model model, HttpServletRequest request, HttpServletResponse response) {
        CookieUtil.add(response, SecureParam.STATE, stateFactory.create(), Period.DAY);
        model.addAttribute("isAuthorised", ObjectParser.parse(auth.isAuthenticated()));
        model.addAttribute("error", ObjectParser.parse(request.getParameter(ReqParam.ERROR)));
        return "index.min";
    }
